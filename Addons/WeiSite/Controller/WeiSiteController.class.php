...
class WeiSiteController extends BaseController {
	...
	function index() {
		...
		foreach ( $category as &$vo ) {
			...
			empty($vo['url']) && $vo['url'] = addons_url('WeiSite://WeiSite/plist', array('cate_id'=>$vo['id']));
		}
	...
	public function plist()
	{
		...
		$pd = new ProductModel();
		$products = $pd->plist($map);
		foreach($products as & $x)
		{
			$x['img1'] = get_cover_url($x['img1']);
			$x['img2'] = get_cover_url($x['img2']);
			$x['img3'] = get_cover_url($x['img3']);
		}
		// exit(json_encode($products));
		$this->assign('products', $products);

		$this->_footer();
		$this->display(ONETHINK_ADDON_PATH . 'WeiSite/View/default/Product/' . 'plist.html');
	}
	// 商品详情
	public function pdetail()
	{
		...
		{
			exit('error can not found the product');
		}
		$map['token'] = get_token();
		$map['id'] = $_REQUEST['id'];

		$pd = new ProductModel;
		$detail = $pd->productDetail($map);

		$detail['img1'] = get_cover_url($detail['img1']);
		$detail['img2'] = get_cover_url($detail['img2']);
		$detail['img3'] = get_cover_url($detail['img3']);

		// exit(json_encode($detail));

		$this->assign('detail', $detail);
		$this->_footer();
		$this->display(ONETHINK_ADDON_PATH . 'WeiSite/View/default/Product/' . 'pdetail.html');
	}
